...
export function metrics(items:StockItem[],orders:Order[]){
...
export function healthScore(items:StockItem[],orders:Order[]):number { const m=metrics(items,orders); let s=100;s-=Math.min(20,m.prep*2);s-=Math.min(15,m.old);s-=Math.min(10,m.missing);s-=Math.min(10,orders.filter(x=>x.deadline==='Today').length*3);return Math.max(45,Math.round(s)); }
export function advanceStatus(item:StockItem):StockItem { const i=FLOW.indexOf(item.status); const next=FLOW[Math.min(FLOW.length-1,Math.max(0,i+1))] ?? item.status; return {...item,status:next,action:next==='Photographed'?'Create listing':next==='Live'?'Monitor listing':next==='Sold'?'Pack order':next==='Dispatched'?'Archive sale':'Complete'}; }
export function sourceCheck(buy:number,sale:number,condition:string,auth:string){ const conservative=sale*.82,profit=sale-buy-1.5,conservativeProfit=conservative-buy-1.5,roi=buy?profit/buy:0; let score=45+Math.min(22,Math.max(0,profit))+Math.min(16,Math.max(0,roi*4))+(condition==='Excellent'?12:condition==='Very good'?8:condition==='Good'?2:-25)+(auth==='High'?5:auth==='Medium'?0:-30);score=Math.max(0,Math.min(100,Math.round(score)));let decision=score>=85?'BUY':score>=70?'BUY SELECTIVELY':score>=55?'CAUTION':'PASS';if(condition==='Needs attention'||auth==='Low'||conservativeProfit<10||buy>10)decision='PASS';return {decision,score,profit,conservativeProfit,roi,maxBuy:Math.max(0,Math.min(10,conservative-1.5-15))}; }
export function normaliseData(raw:Partial<AppData>,fallback:AppData):AppData { return {version:APP_VERSION,items:Array.isArray(raw.items)?raw.items:fallback.items,orders:Array.isArray(raw.orders)?raw.orders:fallback.orders}; }
